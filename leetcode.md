...
+ 确定节点属于哪一层：     
+ T104 树的高度：递归思想。左子树的高度/右子树的高度+1，结束条件，节点为空，高度为0。        
+ T110 判断平衡二叉树：辅助函数：计算树的高度。然后递归思想，当前节点的左子树-右子树的高度差小于1并且左子树和右子树都是平衡二叉树。       
+ T543 树的直径（两节点最长路径）：求解每个节点的高度，直径为节点左子树高度+节点右子树高度的最大值，所以递归函数求解树的高度需要传递最大值作为参数，最大值改变则更新最大值。                 
+ T112 判断路径和是否等于一个数：递归思想。左子树或者右子树满足目标值减去当前节点值等于0，结束条件是当前节点值等于目标值且为叶子节点。              
+ T437 统计路径和等于一个数的路径数量：递归思想，辅助函数：计算当前节点的路径数量。所有路径等于根节点的路径数量和左右子树的路径数量之和。计算当前节点的路径数量也是递归函数，当前值等于目标值表示符合条件返回1，否则递归检查左右子树。               
+ T111 树的最小深度：递归思想，左子树或者右子树高度的较小值+1，特殊情况，只有左子树或者只有右子树，则取两个子树的高度和+1。          
+ T404 二叉树左叶子节点和：递归思想，返回左子树和右子树所有左叶子节点的和，递归遍历每个节点，如果是左叶子节点则更新累加的返回值。       
+ T124 二叉树的最大路径和：递归思想。递归函数定义为返回经过节点的左子树或者右子树的最大值，如果左子树或者右子树小于0则置为0，计算过程中更新总的最大值。                          
+ T101 树的对称：递归思想，左右子树相等，将一棵树拆成2棵树，分别比较每个节点是否相同。              
+ T863 等于目标距离的所有树节点：          
...
+ 结论        
  ...
  4.1) DFS树和图的区别：树是有向的图，图可能是无向+有环，所以树DFS遍历（前序/中序/后序遍历）时不需要标记节点是否被访问过，每个节点，但是图需要，访问过的节点不需要再访问了。   https://leapgraph.com/tree-vs-graph-data-structures            
   5）BFS：广度优先，想象队列为水平类型，先进先出，所以适合BFS?      
   6） DFS和回溯的区别：DFS是不剪枝的回溯。回溯是DFS作用于隐式的树。 DFS是回溯特殊的一种方式。 回溯解决更通用的问题，如八皇后和数独问题。                        
...
+ T70 爬楼梯问题：     
+ T139 单词切断问题：          
+ T121 股票买卖问题：归纳法，状态为天数、交易次数和持有股票状态。定义状态转移方程： d[i][k][0] = max(d[i-1][[k]][0], d[i-1][k-1][1] + price[i]), d[i][k][1] = max(d[i-1][[k]][1], d[i-1][k-1][0] - price[i]), i表示天数，k表示最多交易次数，最后一个下标0表示没持股票，1表示持有股票，d[3][2][0]表示第3天没有股票最多交易2次的利润最大值。k固定为1时，可以去掉一维变为二维，base case：d[-1][0] = 0，表示没有交易，利润为0，d[-1][1]=负无穷，表示没有交易持有股票是不可能的，利润为负无穷。        
+ T53  子数组最大的连续和：归纳法，状态是数组下标，一维DP，dp[n]表示nums[0..n-1]数组里面子数组最大的连续和。通俗来讲这个问题就变成了，第 i 个子组合的最大值可以通过第i-1个子组合的最大值和第 i 个数字获得，如果第 i-1 个子组合的最大值没法给第 i 个数字带来正增益，我们就抛弃掉前面的子组合，自己就是最大的了。定义状态转移方程：dp[n] = max(dp[n-1] + nums[n], nums[n]) ，有的题目结果是dp[i],本题的结果是dp[0]..dp[i]中的最大值。            

## **链表**       
T206 链表反转：
+ 双指针：2个指针从头开始局部反转，一直到尾节点。              
+ 递归方式：定义递归函数：输入一个节点 head，将「以 head 为起点」的链表反转，并返回反转之后的头结点。    
![leetcode](png/leetcode-list1.png)
![leetcode](png/leetcode-list2.png)        
![leetcode](png/leetcode-list3.png)             
+ 迭代方式：利用栈，将所有节点入栈，然后逐个出栈。       
+ 双链表：双链表求解是把原链表的结点一个个摘掉，每次摘掉的链表都让他成为新的链表的头结点，然后更新新链表。            
+ T234 回文链表：
+ 递归反转：通过递归反转链表，然后两个链表比对是否一致。            
+ 递归后序遍历：通过将链表后序遍历，然后返回的时候和原链表的数据比对，检查数据是否一致。      
+ 双指针优化空间复杂度：             
+ T24 两两交换链表的节点：       
+ 栈：每入栈2个节点，然后进行出栈，出栈所有节点后再进行入栈操作，如此反复。       
+ 递归反转：参考链表递归反转的方式。          
+ T160 相交链表：      
+ 暴力法： 对链表A中的每一个结点A[i]，遍历整个链表B，并检查链表B中是否存在结点和A[i]相同。               
+ 双指针法：这个算法也浪漫了吧，错的人迟早会走散，而对的人迟早会相逢！pA和pB两个指针，走完各自的链表继续走对方的链表，如果存在交点，必定会在过程中相遇。                   
+ 哈希表法： 遍历链表A并将每个结点的地址/引用存储在哈希表中。然后检查链表B中的每一个结点B[i]是否在哈希表中。若在，则B[i]为相交结点。                                    
...
参考： https://leetcode-cn.com/problems/queue-reconstruction-by-height/solution/406gen-ju-shen-gao-zhong-jian-dui-lie-tan-xin-suan/      
T665 修改一个数成为非递减数组：遍历数组，初始count = 0，如果当前元素值比它下一个元素值大，则count += 1，当count > 1时，直接返回false。另外，在遍历数组的过程中，如果遇到 “特殊情况”，可以直接返回false；当循环正常结束则返回true。特殊情况有2种：nums[i] < nums[i-1]时，可以选择把nums[i] 放大或者 把 nums[i-1] 缩小，并且保证后面的数保持递增或者递减。         
T53 子数组最大的连续和：
+ 方法1：暴力法：     
+ 方法2：动态规划法：参考动态规划T53的讲解。         
+ 方法3：贪心法：     
+ 方法4：分治法：     
...
## **回文**             
+ 说明：寻找回文串是从中间向两端扩展，判断回文串是从两端向中间收缩。       
...
+ T88 归并两个有序数组：   
+ T763 分隔字符串划分字母区间：记录每个字符最后出现的位置，然后遍历每个字符，遍历过程中下标等于字符最后出现位置则进行分割，遍历过程中如果发现中间有字符在更后面的位置，则更新最后出现的位置。利用双指针，第一个指针记录开始位置，后一个指针记录字符串分割位置，当下标等于字符最后出现位置时，表示可以进行分割。                   
